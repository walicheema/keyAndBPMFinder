...
def get_tempo(request):
    try:
        ...
        mp3_url = request.data.get('mp3URL')

        if not mp3_url:
            return Response({'error': 'No mp3URL provided'}, status=400)

        # Download the MP3 file from the provided URL
        response = requests.get(mp3_url)
        if response.status_code != 200:
            return Response({'error': 'Failed to download MP3 file'}, status=500)
...
def get_key(request):
    try:
        ...
        wav_file = wave.open('my_audio.wav', 'wb')
        ...
        # Create a Tonal_Fragment object
        tonal_fragment = Tonal_Fragment(y, sr)

        # Get the key using the Tonal_Fragment class
        key_name = tonal_fragment.key

        return Response({'key': tonal_fragment.print_key()})
    except Exception as e:
        return Response({'error': str(e)}, status=500)
...
class Tonal_Fragment(object):
    ...
    def chromagram(self, sr, title=None):
